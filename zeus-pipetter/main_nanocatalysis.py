...
def generate_event_list_for_surface_detection(path_for_stock_solution: str = path_for_reactions):
    ...
    for solution in stock_solution_list:
        ...
        event_temp.asp_liquidClassTableIndex = 12  # use pLLD for surface detection
        event_temp.disp_liquidClassTableIndex = 12
        event_temp.asp_liquidSurface = 1600
        event_temp.asp_lldSearchPosition = 1600
        event_temp.disp_liquidSurface = 2000
...
liquid_info_in_stock = pln.run_events_chem_nps(zm=zm, pt=pt, logger=logger,
                                           start_event_id=0,
...
event_dataframe_chem, event_list_chem = \
    pln.generate_event_object(logger=logger,
                              ...
                              sheet_name='Reactions_0404', usecols='B:K',
                              is_pipeting_to_balance=False, is_for_bio=False)
time.sleep(1)


# save the event list in pickle file and later load from this file
path_for_pickle_event_list = f'NPs\\event_list_NPs_{datetime.now().strftime("%Y%m%d_%H%M%S")}.pickle'
with open(path_for_pickle_event_list, 'wb') as f:
