...
def measure_one_spectrum_by_pyautogui(sample_name):
    ...
    # time.sleep(0.1)
    pyautogui.write(sample_name, interval=0.03)
    # time.sleep(0.1)
...
async def main(event_list = None):
    ...
    for num, event in enumerate(event_list):
        ...
        if num != 0:
            print(f'Measuring spectrum...{num}')
            sample_name = str(num)
        ...
        time.sleep(0.5)
        nd.open_lid()
        time.sleep(0.8) # this allows more time for the lid to fully open
...
if __name__ == '__main__':
    ...
    # measure_one_spectrum_by_pyautogui('test')
    ...
    # asyncio.run(m1(event=events_for_measurement[0]))
