...
class ZeusLTModule(object):
    ...
    def send_command(self, cmd, led=None, characteristic_str='er'):
        ...
        while 1:
            ...
            if (characteristic_str in line_here): # This assumes the error_code lines from all the commends have an 'er' in it.
                print(f'error_code received: {line_here}')
                break
    ...
    def asp(self,asp_volume = 5000,
        ...
        error_code = self.send_command(f"00ALid{self.id:04d}av{asp_volume:05d}oa{overasp:05d}"
                          f"fr{flow_rate:05d}ss{stop_speed:05d}qm{qpm:01d}"
                          f"bi{pressure_sensor:01d}qc{qpm_clot:04d}qf{qpm_foam:04d}"
                          f"qe{qpm_empty:04d}to{time_after_asp:03d}", led=1)
...
if __name__ == "__main__":
    ...
    zlt = ZeusLTModule(id=815, COMport='COM5', COM_timeout=0.1, baudrate=19200)
    # time.sleep(1)
    # zlt.tip_pick_up()
    # time.sleep(5)
    # zlt.asp()
