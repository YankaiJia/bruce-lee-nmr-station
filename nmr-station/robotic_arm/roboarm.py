...
class RobotArm:
    ...
    def go_to_safe(self):
        ...
        if (input('CAUTION! Arm going to SAFE_POS. COLLISION MAY OCCUR! [y/n]?')
                in ["yes", "YES", 'Y', 'y']):

            # time.sleep(2)
            if input('Reconfirm going to SAFE_POS. COLLISION MAY OCCUR! [y/n]?') in ["yes", "YES", 'Y', 'y']:
                self.robo.MoveJoints(safe_position[0],
                                safe_position[1],
                                safe_position[2],
                                safe_position[3],
                                safe_position[4],
    ...
    def refresh_j4(self):
        ...
        if  math.isclose(cur_joints[3], 0, abs_tol= 0.5) and \
            math.isclose(cur_joints[5], 0, abs_tol= 0.5) and \
            cur_joints[5] != 0 and \
            cur_joints[3] != 0 and \
            math.isclose(cur_joints[3]+cur_joints[5],0, abs_tol= 0.3):
    ...
    def go_to_high_location(self, target_cart):
        ...
        assert target_cart[2]>= HIGH_Z, (f'Target cart NOT high enough. '
