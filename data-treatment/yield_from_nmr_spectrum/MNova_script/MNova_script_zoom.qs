...
const sol_ref_peak = 4.0; //sovlent peak real ppm
const sol_peak_search_from_ppm = 2;//Search range for REF peak pick
const sol_peak_search_to_ppm = 5 ;


// Global variables
const REF_peak_ppm = sol_ref_peak;//Real peak ppm
const REF_peak_search_from_ppm = sol_peak_search_from_ppm;//Search range for REF peak pick
const REF_peak_search_to_ppm = sol_peak_search_to_ppm;

var dataExtension = "/1/fid";// Global variable for the file extension
...
//*****************************REFERENCING*******************************************//
    ...
    // solvent_peaks_ppm_for_reference = solvent_peak_ppm[0];
    // if (solvent_peak_ppm.length==0){
	// 	MessageBox.critical("No solvent peak is found!");
	// 	}
		
    // if (solvent_peak_ppm.length>=2){
	// 	//MessageBox.critical("Two solvent peaks are found! Using the left most one. ");
	// 	//MessageBox.critical(solvent_peak_ppm);
		
	// 			solvent_peaks_ppm_for_reference =Math.min.apply(null, solvent_peak_ppm);
    ...
    // p.setParameter("ref[1].Shift", solvent_peaks_ppm_for_reference, REF_peak_ppm);
