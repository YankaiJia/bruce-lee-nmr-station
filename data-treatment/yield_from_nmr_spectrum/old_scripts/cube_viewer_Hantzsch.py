...
def plot_3d_dataset_as_cube(x_raw, y_raw, z_raw, k_raw, substance_titles = ('Alcohol', 'HBr', 'Temperature'),
    ...
    if is_label_points:
        ...
        print("-----------------------------")
        xs, ys, zs = xs.tolist(), ys.tolist(), zs.tolist()
        # mlab.text3d(xs, ys, zs, [1]*len(xs), scale=0.03, color=(0, 0, 0))
        spectrum_name = [str(i) for i in spectrum_name]
        for i in range(len(xs)):
            mlab.text3d(xs[i], ys[i], zs[i], spectrum_name[i], scale=0.03, color=(0, 0, 0))
...
if __name__ == '__main__':
    # get data from path
    path = "D:\\Dropbox\\robochem\\data\\BPRF\\HPLC_results\\Hantzsch_80\\average_results_HPLC.csv"
    df = pd.read_csv(path)
    ...
    x_raw, y_raw, z_raw = df['c#methoxybenzaldehyde'], df['c#ethyl_acetoacetate'], df['c#ammonium_acetate']

    k_raw_ls = [df['HE_y_HPLC'], 
                df['THP_y_HPLC'], 
                df['19k_y_HPLC'], 
                ]
    title_ls = ['HE_y_HPLC', 'THP_y_HPLC', '19k_y_HPLC']
    substance_titles=('methoxybenzaldehyde', 'ethyl_acetoacetate', 'ammonium_acetate'),
    spectrum_name_ls = [df['global_index'].tolist()] * len(k_raw_ls)
    k_upper_bound_ls = [200, 200, 200]
