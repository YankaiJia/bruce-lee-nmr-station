...
# ax = axs[3]
# color = 'C2'
# litdata = np.loadtxt('misc-scripts/literature_spectra/ammer-sailer-riedle-2012-E12.txt', delimiter='\t')
# ax.plot(litdata[:, 0], litdata[:, 1], color=color, linewidth=2, alpha=0.9, label='Ammer et al. (2012), E3+')
# ax.fill_between(x=litdata[:, 0], y1=0, y2=litdata[:, 1], color=color, alpha=0.4)
#
# ax = axs[4]
# color = 'C3'
# litdata = np.loadtxt('misc-scripts/literature_spectra/ammer-sailer-riedle-2012-E3.txt', delimiter='\t')
# ax.plot(litdata[:, 0], litdata[:, 1], color=color, linewidth=2, alpha=0.9, label='Ammer et al. (2012), E12+')
# ax.fill_between(x=litdata[:, 0], y1=0, y2=litdata[:, 1], color=color, alpha=0.4)


ax = axs[3]
data = np.loadtxt('D:/Docs/Dropbox/Lab/pink-mystery/dft-monomer-radical-cation-uv-60-states.txt', skiprows=4)
ax.plot(data[:, 0], data[:, 1], label='Radical cation, theory', linewidth=2, alpha=0.5)
ax.fill_between(x=data[:, 0], y1=0, y2=data[:, 1], color='C0', alpha=0.4)
ax.set_ylim(0, 11845)
...
ax = axs[1]
color='C1'
data = np.loadtxt('D:/Docs/Dropbox/Lab/pink-mystery/dft-monomer-radical-uv-60-states.txt', skiprows=4)
maxy = np.max(data[:, 1])
...
ax = axs[2]
color='C2'
data = np.loadtxt('D:/Docs/Dropbox/Lab/pink-mystery/dft-monomer-cation-uv-60-states.txt', skiprows=4)
maxy = np.max(data[:, 1])
