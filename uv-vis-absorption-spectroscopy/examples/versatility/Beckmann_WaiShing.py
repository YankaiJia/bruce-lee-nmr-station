from versatility_examples import *

experiment_name = f'nanodrop-spectrometer-measurements/versatility_test/Beckmann_WaiShing/'

cut_from = 0

construct_calibrant(
    cut_from=cut_from,
    lower_limit_of_absorbance=0.007,
    concentration_column_name='concentration',
    do_plot=False,
    calibration_source_filename='2023-10-18_17-34-17_UV-Vis_benzophenone',
    calibrant_shortnames=['benzophenone'],
    ref_concentrations=[0.0005],
    max_concentrations=[0.00025],
    experiment_name=experiment_name,
    no_right_edge_subtraction=True
)


construct_calibrant(
    cut_from=cut_from,
    lower_limit_of_absorbance=0.007,
    concentration_column_name='concentration',
    do_plot=False,
    calibration_source_filename='2023-10-19_15-33-12_UV-Vis_benzophenone_oxime',
    calibrant_shortnames=['benzophenone_oxime'],
    ref_concentrations=[0.0005],
    max_concentrations=[1],
    experiment_name=experiment_name,
    no_right_edge_subtraction=True,
)

construct_calibrant(
    cut_from=cut_from,
    lower_limit_of_absorbance=0.007,
    concentration_column_name='concentration',
    do_plot=False,
    calibration_source_filename='2023-10-19_11-40-31_UV-Vis_benzanilide',
    calibrant_shortnames=['benzanilide'],
    ref_concentrations=[0.0005],
    max_concentrations=[1],
    experiment_name=experiment_name,
    no_right_edge_subtraction=True,
)


sp = process_wellplate_spectra.SpectraProcessor(
    folder_with_correction_dataset='uv-vis-absorption-spectroscopy/microspectrometer-calibration/'
                                   '2022-12-01/interpolator-dataset/')
sp.nanodrop_lower_cutoff_of_wavelengths = 220
calibration_folder = data_folder + experiment_name + 'microspectrometer_data/calibration/'
# process_plate(sp, dilution_factor=5000,
#               plate_folder=f'{data_folder}{experiment_name}2023-10-25_16-51-53_UV-Vis_diphenylisobenzofuran_crude.csv',
#               well_ids=[7],
#               cut_from=cut_from,
#               cut_to=300,
#               calibrant_shortnames=['dimethyl_acetylenedicarboxylate', 'diphenylisobenzofuran', 'product'],
#               calibration_folder=calibration_folder,
#               experiment_name=experiment_name,
#               do_plot=True,
#               use_line=False,
#               get_errors_from_fit=True,
#               upper_bounds=[np.inf, np.inf, np.inf])
process_plate(sp, dilution_factor=1700,
              plate_folder=f'{data_folder}{experiment_name}2023-10-19_15-48-36_UV-Vis_crude.csv',
              well_ids=[12],
              cut_from=cut_from,
              cut_to=300,
              calibrant_shortnames=['benzophenone', 'benzophenone_oxime', 'benzanilide'],
              calibration_folder=calibration_folder,
              experiment_name=experiment_name,
              do_plot=True,
              use_line=False,
              get_errors_from_fit=True,
              upper_bounds=[np.inf, np.inf, np.inf])