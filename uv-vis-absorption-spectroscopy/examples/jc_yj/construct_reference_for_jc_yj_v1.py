...
experiment_name = f'test_experiment_1/'

# # Settings in case of styrene
# calibrant_shortname = 'STYRENE'
# # ref_concentration = 0.0218
# ref_concentration = 0.0109

# # Settings in case of epox
# calibrant_shortname = 'EPOX'
# # ref_concentration = 0.011
# ref_concentration = 0.0055

# Settings in case of alde
calibrant_shortname = 'ALDE'
# ref_concentration = 0.0062
ref_concentration = 0.0031

# # Settings in case of Substrat

calibration_folder = data_folder + experiment_name + 'microspectrometer_data/calibration/'
plate_folder = data_folder + 'raw_calibration_data/2023-09-12-PBAS-reference.csv'
one_calibrant_df = pd.read_csv(data_folder + 'raw_calibration_data/2023-09-12-PBAS-reference.txt')
one_calibrant_df['nanodrop_col_name'] = one_calibrant_df['nanodrop_col_name'].astype(str)
one_calibrant_df = one_calibrant_df[one_calibrant_df['substance'] == calibrant_shortname]
# iterate over unique concentrations and write a list of nanodrop_col_names for each concentration
# make a dataframe with unique concentrations and nanodrop_col_names
df_unique = pd.DataFrame(columns=[concentration_column_name, 'nanodrop_col_name'])
for concentration in one_calibrant_df[concentration_column_name].unique():
    nanodrop_col_names = list(one_calibrant_df[one_calibrant_df[concentration_column_name] == concentration]['nanodrop_col_name'].values)
    print(nanodrop_col_names)
    df_unique = df_unique.append({concentration_column_name: concentration, 'nanodrop_col_name': nanodrop_col_names}, ignore_index=True)

one_calibrant_df = df_unique
...
def load_spectrum_by_df_row(row):
    wavelengths = nanodrop_df['wavelength'].to_numpy()
    absorbances = np.stack([nanodrop_df[nd_name].to_numpy() for nd_name in row['nanodrop_col_name']]).mean(axis=0)
