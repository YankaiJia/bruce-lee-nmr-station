...
smth_func = SmoothBivariateSpline(train_x, train_y, train_z, s=int(round(len(train_x)*2)))
...
plt.figure()
plt.pcolormesh(grid_x, grid_y, np.sqrt(np.abs(grid_z)), cmap='viridis', shading='linear')
