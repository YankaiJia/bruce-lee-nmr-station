...
# only use data with maximum ic001
df_results = df_results[df_results['ic001'] > 0.290]
...
class MyModel(HasTraits):
    ...
    def update_plot(self):
        ...
        if self.plot is None:
            self.plot = self.scene.mlab.contour3d(xnew, ynew, znew, wnew, extent=[0.0, 0.30, 0.0, 0.30, 0.0, 0.30],
            ...
            self.scene.mlab.xlabel(f'Product concentration times 5')
            ...
            # convert to attractor analysis representation
            ks_copy = ks.copy()
            xs_copy = xs.copy()
            ys_copy = ys.copy()
            zs_copy = zs.copy()
            prod_c = np.zeros_like(ks)
            for i in range(xs.shape[0]):
                lowest_abundance_sub = min([xs[i], ys[i], zs[i]])
                prod_c[i] = ks[i] * lowest_abundance_sub
            ks_copy = ks_copy/ks_copy * 0.1
            xs_copy = prod_c*5
            ys_copy -= prod_c
            zs_copy -= prod_c


            self.plot_points.mlab_source.reset(x=xs_copy, y=ys_copy, z=zs_copy, scalars=ks_copy)
