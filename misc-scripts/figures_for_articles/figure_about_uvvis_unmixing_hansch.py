...
def spectrum_to_concentration_local(sp, target_spectrum_input, calibration_folder, calibrant_shortnames,
                              ...
                              cut_to=False, ignore_pca_bkg=True, return_errors=False):  # upper_bounds=[np.inf, np.inf]
    color_list = [f'C{i}' for i in range(10)] + [f'C{i}' for i in range(10)] + ['black'] * 5
    # color_list[8] = 'C4'
    # color_list[9] = 'C5'
    ...
    def func(*args):
        ...
        return sum([calibrant_coefficients[i] * calibrants[i]['reference_interpolator'](xs) for i in
                    range(len(calibrant_coefficients))]) \
    ...
    vertical_shift = popt[-4]
    line_experimental = ax.plot(wavelengths, target_spectrum - vertical_shift, label='Measured spectrum\nof crude mixture', color='black', linewidth=3, alpha=0.5)
    ...
    label_calibrant_shortnames = calibrant_shortnames
    ...
    for calibrant_index in range(len(calibrant_shortnames)):
        ...
        lines_components.append(
            ax.plot(wavelengths, func(wavelength_indices, *cpopt), label=label_calibrant_shortnames[calibrant_index], alpha=0.5, color=color_list[calibrant_index])
        )
        ax.fill_between(x=wavelengths, y1=0, y2=func(wavelength_indices, *cpopt), alpha=0.1, color=color_list[calibrant_index])
...
def process_plate(sp, dilution_factor,
    ...
    for well_id in well_ids:
        ...
        concentrations_here = spectrum_to_concentration_local(sp, target_spectrum_input=spectrum,
                                                             fig_filename=data_folder + f'BPRF/2024-02-16-run01/results/illustrations/{plate_name}-well{well_id:02d}',
                                                             do_plot=do_plot,
                                                             calibration_folder=data_folder + 'BPRF/2024-01-17-run01/' + 'microspectrometer_data/calibration/',
                                                             calibrant_shortnames=['methoxybenzaldehyde', 'HRP01', 'dm35_8', 'dm35_9', 'dm37', 'dm40_12', 'dm40_10', 'ethyl_acetoacetate', 'EAB', 'bb017', 'bb021', 'dm70'],
...
process_plate(sp, dilution_factor=500,
              plate_folder=f'{data_folder}{experiment_name}2024-02-18_17-48-07_UV-Vis_plate74.csv',
              well_ids=[16],
              cut_from=80,
              cut_to=False,
