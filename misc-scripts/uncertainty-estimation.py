...
def zeus_uncertainty_from_file(tfile, do_plot=True):
    ...
    return tip_type, np.array(statistical_data), error_interpolator

def get_error_interpolators_from_all_measurement_files(zeus_volume_validation_measurements_files):
...
# Measurement
vial_cross_section = ufloat(6, 0.05) # in mm
absorbance = concentration_in_second_well_after_three_operations * \
             (dilution_transfer_volumes[1] + dilution_transfer_volumes[2]) / vial_cross_section
