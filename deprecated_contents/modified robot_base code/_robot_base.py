...
"""
    ...
    @staticmethod
    #pylint: disable=unused-argument
    def _handle_socket_rx_chatgpt(robot_socket, rx_queue, logger):
        """Handles socket RX safely, ensuring only full messages are processed."""

        remainder = ''  # Stores incomplete messages
        while True:
            try:
                raw_responses = robot_socket.recv(1024)

                if not raw_responses:
                    logger.warning("⚠ Empty response received, possible disconnection.")
                    return

                responses = raw_responses.decode('ascii').split('\0')

                # Reattach the previous remainder
                if remainder:
                    responses[0] = remainder + responses[0]

                # Store last part as remainder if incomplete
                if responses[-1]:
                    remainder = responses[-1]
                else:
                    remainder = ''
                ...
                logger.error(f"❌ Socket RX Error: {e}")
                return
    ...
    def _check_command_threads_old(self):
        ...
        if not (self._command_rx_thread and self._command_rx_thread.is_alive()):            ## if the rx_thread drops, do not raise error and restart it.
            # raise InvalidStateError('No command rx thread, are you in monitor mode?')
            self.logger.error('command thread is dropped, relaunching...')
            time.sleep(1)
            ...
            try:
                ...
                self.logger.error("Command thread relaunched successfully.")
    ...
    def _shutdown_command_socket(self):
        """Safely shut down the command socket and clean up associated resources."""
        with self._main_lock:
            self.logger.info("Shutting down command socket...")

            # Stop and clear command threads if they exist
            if self._command_rx_thread:
                self.logger.debug("Stopping command receive thread...")
                self._command_rx_thread = None  # Allow garbage collection

            if self._command_tx_thread:
                self.logger.debug("Stopping command transmit thread...")
                self._command_tx_thread = None  # Allow garbage collection

            # Clear command queues
            with self._command_rx_queue.mutex:
                self._command_rx_queue.queue.clear()
            with self._command_tx_queue.mutex:
                self._command_tx_queue.queue.clear()

            # Close the command socket
            if self._command_socket:
                try:
                    self.logger.debug("Closing command socket...")
                    self._command_socket.close()
                except Exception as e:
                    self.logger.error(f"Error while closing command socket: {e}")
                finally:
                    self._command_socket = None
            ...
            self.logger.info("Command socket shut down successfully.")

    def _restart_command_socket(self, max_retries=5, retry_delay=2):
        """Retry reconnecting to the command socket up to max_retries times."""
        with self._main_lock:
            for attempt in range(max_retries):
                self.logger.warning(f"Attempt {attempt + 1}/{max_retries} to reconnect...")
                try:
                    self._command_socket = self._connect_socket(self.logger, self._address, self._port)
                    self.logger.info(f"Reconnected to {self._address}:{self._port}")

                    # Restart threads
                    self._command_rx_thread = threading.Thread(
                        target=self._handle_socket_rx,
                        args=(self._command_socket, self._command_rx_queue, self.logger),
                        daemon=True
                    )
                    self._command_rx_thread.start()

                    self._command_tx_thread = threading.Thread(
                        target=self._handle_socket_tx,
                        args=(self._command_socket, self._command_tx_queue, self.logger),
                        daemon=True
                    )
                    self._command_tx_thread.start()
                    return  # Exit function if reconnection is successful

                except Exception as e:
                    self.logger.error(f"Reconnection failed: {e}. Retrying in {retry_delay} seconds...")
                    time.sleep(retry_delay)

            self.logger.critical("Failed to reconnect after multiple attempts. Manual intervention required.")

    def _check_command_threads_gpt(self):
        """Check and restart command threads if they are not alive."""

        ### 1️⃣ Check & Restart Command Response Handler Thread
        if not (self._command_response_handler_thread and self._command_response_handler_thread.is_alive()):
            self.logger.warning('Command response handler thread dropped. Attempting restart...')

            try:
                self._command_response_handler_thread = self._launch_thread(target=self._command_response_handler,
                                                                            args=())
                self.logger.info("✅ Command response handler thread relaunched successfully.")
            except Exception as e:
                self.logger.error(f"❌ Failed to restart command response handler thread: {e}")

        ### 2️⃣ Skip RX/TX Thread Check if in Offline Mode
        if self._offline_mode:
            return

        ### 3️⃣ Check & Restart Command RX Thread
        if not (self._command_rx_thread and self._command_rx_thread.is_alive()):
            self.logger.warning('Command RX thread dropped. Attempting restart...')

            # Ensure socket is still open
            if self._command_socket is None:
                self.logger.error("❌ Cannot restart RX thread: Command socket is closed.")
                return

            # Clean up the previous thread properly
            if self._command_rx_thread:
                self._command_rx_thread.join(timeout=self.default_timeout)
                self._command_rx_thread = None

            try:
                # Restart RX thread with a buffered message handler
                self._command_rx_thread = self._launch_thread(
                    target=self._handle_socket_rx,
                    args=(self._command_socket, self._command_rx_queue, self.logger)
                )
                self.logger.info("✅ Command RX thread relaunched successfully.")
            except Exception as e:
                self.logger.error(f"❌ Failed to restart command RX thread: {e}")

        ### 4️⃣ Check & Restart Command TX Thread
        if not (self._command_tx_thread and self._command_tx_thread.is_alive()):
            self.logger.warning('Command TX thread dropped. Attempting restart...')

            if self._command_socket is None:
                self.logger.error("❌ Cannot restart TX thread: Command socket is closed.")
                return
            ...
            try:
                ...
                self.logger.info("✅ Command TX thread relaunched successfully.")
            except Exception as e:
                self.logger.error(f"❌ Failed to restart command TX thread: {e}")
    ...
    def _check_command_threads(self):
        ...
        if not (self._command_rx_thread and self._command_rx_thread.is_alive()):
            # raise InvalidStateError('No command rx thread, are you in monitor mode?')
            print('No command rx thread, are you in monitor mode?')
            # self.Disconnect()
            a = self.GetRtTargetJointPos()
            print(self.GetRtTargetJointPos())
            print(self.GetRtTargetCartPos())

            self._shut_down_queue_threads()
            self._shut_down_socket_threads()
            self.Disconnect()
            self.Connect(address="192.168.0.100",
            ...
            print(self.GetRtTargetJointPos())
            print(self.GetRtTargetCartPos())
            self._set_monitoring_interval_internal(0.1)  # Enable real-time updates
            self.GetStatusRobot()
            self.ResetError()
            self.ResumeMotion()
            time.sleep(5)
            print(self.GetRtTargetJointPos())
            print(self.GetRtTargetCartPos())
            print(list(self._command_rx_queue))
            print(list(self._command_tx_queue))
        ...
        if not (self._command_tx_thread and self._command_tx_thread.is_alive()):
            # self._command_socket.sendall(b'DeactivateRobot\0')
            # raise InvalidStateError('No command tx thread, are you in monitor mode?')
            print('No command tx thread, are you in monitor mode?')
            # self.Disconnect()
            print(self.GetRtTargetJointPos())
            self._shut_down_queue_threads()
            self._shut_down_socket_threads()
    ...
    def _check_command_threads_new(self):
        """Check and restart command threads if they are not alive."""

        ### 🛑 Check if the socket is valid before proceeding
        if self._command_socket is None or self._command_socket.fileno() == -1:
            self.logger.warning("⚠ Command socket is closed. Reconnecting...")
            try:
                self._reconnect_socket()
                self.logger.info("✅ Command socket reconnected successfully.")
            except Exception as e:
                self.logger.error(f"❌ Failed to reconnect socket: {e}")
                return  # Prevent further execution if reconnection fails

        ### 1️⃣ Check & Restart Command Response Handler Thread
        if not (self._command_response_handler_thread and self._command_response_handler_thread.is_alive()):
            self.logger.warning('Command response handler thread dropped. Restarting...')
            try:
                self._command_response_handler_thread = self._launch_thread(target=self._command_response_handler,
                                                                            args=())
                self.logger.info("✅ Command response handler thread restarted successfully.")
            except Exception as e:
                self.logger.error(f"❌ Failed to restart command response handler thread: {e}")

        ### 2️⃣ Check & Restart Command RX Thread
        if not (self._command_rx_thread and self._command_rx_thread.is_alive()):
            self.logger.warning('Command RX thread dropped. Restarting...')
            ...
            try:
                ...
                self.logger.info("✅ Command RX thread restarted successfully.")
            except Exception as e:
                self.logger.error(f"❌ Failed to restart command RX thread: {e}")

        ### 3️⃣ Check & Restart Command TX Thread
        if not (self._command_tx_thread and self._command_tx_thread.is_alive()):
            self.logger.warning('Command TX thread dropped. Restarting...')
            ...
            try:
                ...
                self.logger.info("✅ Command TX thread restarted successfully.")
            except Exception as e:
                self.logger.error(f"❌ Failed to restart command TX thread: {e}")

    def _check_command_threads_chatgpt(self):
        """Check if command threads are running, shut down and restart the command socket if any have failed."""
        with self._main_lock:
            # Check if command threads are still running
            if (not (self._command_rx_thread and self._command_rx_thread.is_alive())) or \
                    (not (self._command_tx_thread and self._command_tx_thread.is_alive())):
                self.logger.warning("A command thread has stopped unexpectedly. Restarting command socket...")

                # Shut down the command socket
                self._shutdown_command_socket()

                # Restart the command socket
                self._restart_command_socket()

                self.logger.info("Command socket successfully restarted.")
    ...
    def _handle_cycle_end_old(self, response: Message):
    ...
    def _handle_cycle_end(self, response):
        """Handles end-of-cycle messages, ensuring valid timestamp parsing."""
        try:
            # Extract only digits from response.data
            clean_data = ''.join(filter(str.isdigit, response.data))

            # Ensure valid integer conversion
            if clean_data.isdigit():
                timestamp = int(clean_data)
                self.logger.debug(f"✅ Parsed timestamp: {timestamp}")
            else:
                raise ValueError(f"🚨 Malformed timestamp data received: {response.data}")

        except ValueError as e:
            self.logger.error(f"❌ Failed to parse timestamp: {response.data} - {e}")
            timestamp = None  # Handle gracefully

        return timestamp  # Return None if parsing fails
